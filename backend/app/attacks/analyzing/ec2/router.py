...
@router.websocket("/ws/analyzing/aws/ec2")
async def analyze_aws_ec2_policy(websocket: WebSocket):
    await websocket.accept()
    await websocket.send_text(json.dumps({"type": "log", "message": "[AWS EC2 정책 분석] - WebSocket 연결됨. 분석 요청을 기다립니다..."}))
    ...
    try:
        ...
        SCRIPT_PATH = os.path.join(os.path.dirname(__file__), "ec2_analyzer.py")
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H%M%S")
        output_file = f"ec2_analyzer_{timestamp}.md"
